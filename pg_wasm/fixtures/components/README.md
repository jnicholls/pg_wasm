# Component test corpus (`pg_wasm/fixtures/components`)

Each subdirectory is a **standalone** Rust crate (not a workspace member) that builds a WebAssembly **component** for `pg_wasm` regress tests.
...
```bash
cd ../..   # repo root
cd pg_wasm && cargo pgrx regress --resetdb
```
