...
Run after `./build_all.sh`:
  ...
  python3 pg_wasm/fixtures/components/generate_pg_regress_sql.py
"""
...
def main() -> None:
    ...
    error_classes_sql = textwrap.dedent(
        f"""\
        -- Representative SQLSTATE per pg_wasm error class (see src/errors.rs).
        SELECT set_config('pg_wasm.fuel_enabled', 'off', false);
        SELECT set_config('pg_wasm.invocation_deadline_ms', '0', false);
        ...
        BEGIN
            PERFORM wasm.load(
                'ec_perm',
                json_build_object('path', '/no/such/pg_wasm_ec_path.wasm'),
                {opt_limits_default}
            );
